...
class SymEmbedding(Extractor):
    def __init__(
        ...
        init_const: Optional[float] = None
    ) -> None:
        ...
        if out_mlp is not None or self.a_emb_dim != self.out_dim:
            ...
            self.out_mlp = None
        self.emb = PinnedEmbedding(self.n_obj, self.n_emb * self.a_emb_dim, self.gpu, flex=True)
        self.emb.init(std=init_std, constant=init_const)
    ...
    def interpolate(self, idx: Tensor, num_steps: int, embedding_subset: Optional[Tensor] = None) -> None:
        ...
        """
        n = len(idx)
        latents = self.emb(idx.flatten()).view(n, 2, self.n_emb, self.a_emb_dim)  # [n, 2, n_emb, a_emb_dim]
        ...
        self.emb.init(interpolated.flatten(0, 1).flatten(-2, -1))
    ...
    def randomize(self, idx: Tensor, num_random: int, std: float, embedding_subset: Optional[Tensor] = None) -> None:
        ...
        if embedding_subset is not None:
            assert embedding_subset.max() < self.n_emb, "Found embedding index in subset that is out of range"
            latents = self.emb(idx).view(n, self.n_emb, self.a_emb_dim)   # [n, n_emb, a_emb_dim]
        self.n_obj = n * num_random
        self.emb.init(std=std)
        if embedding_subset is not None:
            random_latents = self.emb(idx).view(n, self.n_emb, self.a_emb_dim)   # [n, n_emb, a_emb_dim]
            random_latents[:, embedding_subset] = latents[:, embedding_subset]
            self.emb.init(random_latents.flatten(0, 1))
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        B = len(idx)
        v = self.emb(idx).view(B, self.n_emb, -1)
        if self.optimize_scores_directly:
